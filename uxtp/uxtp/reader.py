...
TP format - works in 512 byte blocks:
...
block 63 - for magtape, start of data files, each file
		   is padded to fill up to the nearest
block 24 - for DecTape
...
class ReadTape():
    ...
    def loadtape(self, tapefile:str) -> bytes:
        ...
        tapef = Path(tapefile)
        if not tapef.exists():
            print(f"Cannot find {tapefile}")
            sys.exit(1)
        if not tapef.is_file():
            print(f"{tapefile} is not a file")
            sys.exit(1)

        # Set DictSiz
        const.DictSiz = const.dirsize(tapef.suffix)
    ...
    def tapeunblock(self, contents:bytes) -> bytes:
        """ Blocks are <twc>BLKSIZ bytes<twc>
    ...
    def parsedir(self, dblks:bytes) -> list[dict]:
        ...
        while offset < len(dblks):
            ...
            if (total & 0xffff) != 0:
                failct += 1
                if failct > 10:
                    print(f'Checksum failure on dir entry {self.entryct} - 10 found abandoning')
                    sys.exit()
            ...
            namestr = namestr.rstrip('\0')
            # look for any embedded nulls
            # this happened when reading from V7
            firstnull = namestr.find('\0')
            if firstnull != -1:
                namestr = namestr[:firstnull]
            dirent['pathstring'] = namestr
